...
class MySQL{
    ...
    static private function connection(){
        if(!is_resource(self::$link) || empty(self::$link)){
            if(($link = mysqli_connect(self::$connection_info['host'], self::$connection_info['user'],
                    self::$connection_info['pass'])) && mysqli_select_db($link, self::$connection_info['db'])){
                self::$link = $link;
...
//            if(self::$info['num_rows'] == 1){
//                $data = array();
//                $data[]= mysqli_fetch_assoc($result);
//            }

             if(!$result)
                 throw new Exception('Error executing MySQL query: '.$qry.'. MySQL error '.mysqli_errno($link).': '.mysqli_error ($link));
    ...
    public function get($table, $select = '*'){
        ...
        if(!($result = mysqli_query($link, $sql))){
            print $sql;
            throw new Exception('Error executing MySQL query: '.$sql.'. MySQL error '.mysqli_errno($link).': '.mysqli_error
                ($link));
            $data = false;
        }else {
    ...
    public function insert($table, $data){
        ...
        if(!mysqli_query($link, $sql)){
            throw new Exception('Error executing MySQL query: '.$sql.'. MySQL error '.mysqli_errno($link).': '
                .mysqli_error($link));
        }else{
    ...
    public function update($table, $info){
        if(empty(self::$where)){
            ...
            if(!mysqli_query($link, $sql)){
                throw new Exception('Error executing MySQL query: '.$sql.'. MySQL error '.mysqli_errno().': '
                    .mysqli_error());
            }else{
                return true;
            }
